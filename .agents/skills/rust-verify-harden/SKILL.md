...
description: >-
    Run the Elph repository's make fmt/check/lint/test gates and fix failures, then audit
    changed and directly related Rust code for memory/resource leaks, deadlock risk, data
    races, structural complexity, and performance regressions. Also audits Elph's Turso
    local datastore for SQLITE_BUSY/"database is locked", WAL/checkpoint configuration,
    connection misuse, sync issues, and corruption risk. Use when preparing Elph for
    merge/release or when reviewing async Rust, TUI/agent runtime, or Turso code. Consult
    current Rust, Tokio, Clippy, and docs.turso.tech guidance when relevant. Flags
    backward-compatibility, legacy, database-design, and structural refactors and asks
    before removing or restructuring them.
metadata:
    scope: project
---
...
**Before each applicable phase, consult current guidance.** Prefer DeepWiki when
the connected DeepWiki capability is available. If it is unavailable, say so in
the report and use the official Rust/Clippy/Tokio documentation, docs.rs,
docs.turso.tech, and rustfaq.org instead. Do not fabricate a DeepWiki citation.

- **Edition-specific guidance**: Search for `"Rust edition 2024 idioms"`,
  `"Rust 2021 edition changes"`, or `"Rust edition migration"` to confirm
  current breaking changes and new features.
- **Clippy lint rationale**: For each lint violation, search for
  `"clippy <lint-name>"` or `"Rust clippy <violation>"` to understand the
  intended fix and context.
...
- **Turso/libSQL/SQLite patterns**: Search for `"libsql Rust database is locked"`,
  `"libsql SQLITE_BUSY retry"`, `"SQLite WAL busy_timeout"`,
  `"Turso embedded replica sync"`, `"turso crate MVCC concurrent writes"`, or
  `"libsql connection pool Rust"` depending on the finding. Cross-check
  against <https://docs.turso.tech> (Rust SDK reference) for crate-specific API
  and configuration details — treat it as authoritative for `libsql`/`turso`
  specifics.
...
**If connected DeepWiki results conflict with training data or existing code
patterns, prioritize DeepWiki. If DeepWiki is unavailable, record the evidence
gap and do not present secondary sources as DeepWiki. Use rustfaq.org and
docs.turso.tech for idiom sanity checks and crate-specific configuration.**

## Workflow

### Phase 1 — Baseline and quality gates

1. **Edition check & idiom baseline**
    - Verify `Cargo.toml` has `edition` set (2021 or 2024 recommended; 2015 is obsolete for new code).
    - Consult `"Rust <edition> features and breaking changes"` guidance.
    ...
    - **Elph baseline:** workspace edition is 2024; workspace and publishable
      crate MSRVs can differ. Record each affected package's `rust-version`.
    - **If the project depends on `libsql` or `turso`**: note the crate and
      exact version in `Cargo.toml`, and actual connection modes
      (`Builder::new_local`, remote, replica, or `turso::sync::Builder`). In
      current Elph, `turso = 0.8.0-pre.7` is used for local storage; inspect
      `crates/elph-agent/src/datastore/conn.rs` before making DB claims.

2. **Linting context**: Before running Clippy:
    - Use `make lint` as the source-of-truth command. Do not require
      `cargo clippy --all-targets -- --list`: current `clippy-driver` may
      reject `--list`; use `-W help` only as an optional supported diagnostic.
    - For each violation, consult `"clippy <lint-name> explanation"` or
      `"<lint-name> Rust best practice"` and inspect local `clippy.toml` and
      `#[allow(...)]` rationale.
    - Review custom `clippy.toml` or in-code `#[allow(...)]` attributes; they may reflect intentional trade-offs.

3. Run `make check` (workspace compile). If it fails:
...
4. Run `make lint` (workspace, all targets, `-D warnings`). For each violation:
    - Consult `"clippy <lint-name> fix"` to understand the intended pattern.
...
5. Run `make test` (workspace `cargo nextest run`). For each failure:
...
7. **Edition-specific idioms** (strict for 2024; note for 2021):
    - Consult `"Rust 2024 unsafe block requirements"` and `"RFC 3233 unsafe"`
      to confirm explicit `unsafe {}` wrapping rules.
    - `unsafe` blocks: All `unsafe` operations must be wrapped in an explicit `unsafe {}` block, even inside `unsafe fn` bodies.
    - Temporary scoping: consult `"Rust temporary lifetime changes"` if
      unfamiliar with edition-specific behavior.
    - Reserved keywords: `gen`, `async gen` are reserved in 2024. Don't use them as identifiers.
    - `unsafe extern` blocks: Mark explicitly.
    - `impl Trait` (AFIT): consult `"impl Trait in function arguments 2024"`
      and `"AFIT capture rules"` for current capture semantics.

8. **Error handling** (in new/changed non-test code):
    - Avoid `.unwrap()`/`.expect()` — replace with proper error propagation.
    - Consult `"Rust error handling best practices"` and
      `"unwrap vs question mark"` if unsure about approach.
...
9. **`unsafe` minimization** (in new/changed code):
    ...
    - If `unsafe` is unavoidable, consult `"Rust unsafe <use-case>"` (for
      example `"Rust unsafe FFI"` or `"Rust unsafe performance critical"`) to
      verify the pattern is sound.
...
**Consult current guidance before each applicable subcategory.** If no suitable
source is available, record that limitation and rely on code and test evidence.

1. **Memory/resource leaks**:
    - Consult `"Rc RefCell cycle detection"`, `"Arc Weak back-references"`,
      and `"Rust memory leak patterns"`.
    ...
    - Consult `"tokio::spawn task cleanup"` and `"Tokio JoinHandle patterns"`
      for task lifecycle.
...
2. **Deadlock risk**:
    - Consult `"Rust mutex deadlock patterns"`,
      `"std::sync::Mutex async executor"`, and `"lock ordering discipline"`.
    ...
    - Consult `"tokio::sync::Mutex vs std::sync::Mutex"` to confirm async-safe
      patterns.

3. **Race conditions / data races**:
    - Consult `"unsafe impl Send Sync"`, `"interior mutability thread safety"`,
      `"Rust static mut"`, `"Atomic operations Rust"`, and
      `"check-then-act race window"`.
    - Run `cargo miri test` only when `cargo-miri` and a compatible toolchain
      are already installed. Otherwise report it as unavailable; do not install
      toolchains or components unprompted.
    - Consult `"cargo miri async tests"` and `"loom testing Rust"` for advanced
      testing approaches. Use Loom only if it is already a dependency.
...
4. **Turso/libSQL/SQLite-specific issues** (only when the project depends on `libsql`, `turso`, `rusqlite`, or talks to a Turso/sqld endpoint):
    - Consult (and cross-check docs.turso.tech) `"libsql database is locked"`,
      `"libsql SQLITE_BUSY retry"`, `"SQLite WAL busy_timeout"`,
      `"Turso embedded replica sync error handling"`, and
      `"turso crate MVCC concurrent writes"` as relevant.
    ...
    - **Connection sharing model**: inspect actual crate semantics before
      generalizing. Check whether connections are cloned handles, pooled,
      serialized by an actor, or shared behind synchronization. Flag a raw
      connection hammered concurrently without a documented safe ownership
      model; do not prescribe a crate swap or pool redesign silently.
    - **Embedded replica / sync issues** (`new_remote_replica`,
      `turso::sync::Builder`): check that sync errors are handled, not
      swallowed; that reads after writes have the required freshness semantics;
      and that sync failures have retry/backoff or an explicit degraded-state
      signal. Do not apply replica/crate redesign without Phase 4 approval.
...
5. **Spaghetti code / structural complexity** (identify only, don't restructure yet):
    - Consult `"cyclomatic complexity Rust"`, `"God object anti-pattern"`,
      `"shotgun surgery code smell"`, `"Rust module boundaries"`, and
      `"control flow refactoring patterns"`. Cross-check general idiom
      questions against <https://www.rustfaq.org/en/>.
...
**Consult current guidance for each optimization:**

1. **Hot-path optimizations**:
    - Consult `"Rust clone avoidance"`, `"Cow smart pointers"`,
      `"Arc performance"`, `"Vec with_capacity performance"`,
      `"String reallocation"`, `"Rust async blocking I/O performance"`,
      `"Tokio hot path"`, `"Rust time complexity optimization"`, and
      `"iterator collect performance"` before applying micro-optimizations.
    - For DB-touching hot paths, consult `"prepared statement caching Rust SQLite"`
      and `"libsql batch execute performance"`. Measure before changing an
...
2. **Apply fixes only if**:
    ...
    - current documentation and a measured/profiled result support the pattern.
...
2. **Summarize**:
    ...
    - **Sources used**: list actual URLs/searches and key findings. If DeepWiki
      was unavailable, say so explicitly and list the official Rust,
      Clippy/Tokio, docs.rs, docs.turso.tech, or rustfaq.org sources used.

## Notes

- **Consult DeepWiki when available**: If connected DeepWiki conflicts with
  training data, defer to DeepWiki. Never claim a source was consulted when it
  was not reachable.
...
- **Target editions**: 2024 (strict), 2021 (acceptable with notes), pre-2021
  (flag for upgrade). Check package-level MSRV as well as workspace edition.
...
- **Git**: Commit/push only if explicitly instructed. Never reset or overwrite
  unrelated work during rollback.
...
1. Consult `"Rust 2024 edition best practices"`, `"Tokio concurrency patterns"`,
   `"libsql database is locked"`, and `"SQLite WAL busy_timeout"`. Check
   docs.turso.tech for the Rust SDK reference relevant to the crate/version in
   use. If DeepWiki is unavailable, record that and use official sources.
