...
| **`codegraph`**             | `enabled` (default **false**), `toolTimeoutMs` (default 15000; `0` disables)                                                                                                    | When true: (1) register agent tools `code_search` / `code_impact` / `code_status` / `code_reindex`; (2) pre-TUI interactive offer to index an empty project. `toolTimeoutMs` bounds each `code_*` call — on timeout the tool returns a fallback error and the agent uses `grep` / `read_file` / `shell_exec`. CLI `elph codegraph` always works. See [codegraph.md](./codegraph.md). |
