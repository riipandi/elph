...
- **TUI / ACP deferred load:** `defer_mcp_load` still constructs a registry from merged `mcp.json` (`McpLoadOptions.skip_startup_discovery`). Discovery runs after the UI is up (`bootstrap_mcp_for_session`) or on the first turn. An empty registry is only used if that config load itself fails.
