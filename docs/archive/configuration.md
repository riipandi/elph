...
| **`ui`**                    | `theme`, `themes`, `showThinking`, `autoExpandThinking`, `stickyScroll`, `footerTokenDisplay`, `coloredStatusFooter`, `narrowLogLines`, `allowModeChangeWhileBusy`, `filePicker.showHiddenFiles`  | Appearance + transcript / chrome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **`codegraph`**             | `enabled` (default **false**), `toolTimeoutMs` (default 15000; `0` disables), `maxDbConnections` (default 4), `maxChunkLines` (default 120), `maxFileBytes` (default 524288), `embedBatchSize` (default 64), `dbCommitBatchFiles` (default 200), `embedConcurrency` (default 1) | When true: (1) register agent tools `code_search` / `code_impact` / `code_status` / `code_reindex`; (2) pre-TUI interactive offer to index an empty project. `toolTimeoutMs` bounds each `code_*` call — on timeout the tool returns a fallback error and the agent uses `grep` / `read_file` / `shell_exec`. `maxDbConnections` / `maxChunkLines` / `maxFileBytes` / `embedBatchSize` / `dbCommitBatchFiles` / `embedConcurrency` are indexing knobs (pool size, chunk split, file-size cap, chunk batching, DB-transaction batching, concurrent embedding batches — `1` = sequential). A `0` for `maxDbConnections` / `maxChunkLines` / `maxFileBytes` is treated as the default — `0` would otherwise deadlock `build`/`update` or skip every file. CLI `elph codegraph` always works. See [codegraph.md](./codegraph.md). |
...
`ui.narrowLogLines` (default `true`) controls transcript **log density**: off → every process-log row (tool calls, thinking, assistant response) keeps a blank line above and below. On → collapsed tool-call items are packed together into a grouped log (no blank line between consecutive collapsed tool rows); expanded (accessed) tool-call items, `Thinking`, and AI chat response/assistant items always keep line breaks above and below.
