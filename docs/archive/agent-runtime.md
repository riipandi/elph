...
| Path                    | Behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Auto**                | If context usage exceeds the threshold — checked **before** sending a new prompt (including the upcoming prompt size) and **after** every successful turn — Elph compacts history while the turn is still busy (so the loading indicator stays coherent instead of a frozen turn that emits post-hoc notices after the prompt box reappears) and posts sticky transcript notices (will / running / done or failed). The status row shows **"Auto-compacting history…"** during compaction, so the user can see the agent is actively finalizing history, not stuck. The estimate matches the header context label (session messages + compiled system prompt, counted once), so `thresholdPct` fires exactly when the chrome shows that percentage. The system prompt is only added on top of the message estimate when provider usage is not reused, so the label never double-counts it and cannot read above 100% of the window for a request that actually fits. When a turn errors with a context-limit overflow, Elph compacts and retries once, but only if the retry still fits after compaction. |
| **Turn-error recovery** | When a turn fails with a context-limit overflow — whether the model returns `stop_reason::Error` or the provider rejects the request with a hard context-overflow error — Elph compacts automatically and then submits a **Continue-style recovery prompt once** (not the original text), so the interrupted task resumes without duplicating tool calls that already succeeded. The retry is bounded to a single attempt. The same recovery runs for both error shapes, so the agent keeps working after compaction rather than stopping.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
Product open/resume uses `AgentHarness::restore` (wired from `crates/coding-agent/src/agent/runtime.rs`). Session open also runs `reconcile_session` in `SessionManager`.
...
`create_coding_session_with_events` in `crates/coding-agent/src/agent/runtime.rs` assembles the harness tool list:
