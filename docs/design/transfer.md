# Foreign session transfer (`/transfer`)
...
| Args                            | Behavior                                                            |
| ------------------------------- | ------------------------------------------------------------------- |
| `/transfer claude`              | Resume the newest Claude Code session for the current cwd           |
| `/transfer claude latest`       | Same as no ref (aliases: `continue`, `-c`)                          |
| `/transfer claude <session-id>` | Resume that session directly by native UUID                         |
| `/transfer claude <words>`      | Resume the uniquely-matching session by title (ambiguous → lists ids) |
| `/transfer codex …`             | Same resolution against Codex rollout transcripts (`~/.codex/sessions`) |

The slash palette offers `claude` / `codex` as argument completions.

## How it works (Claude)

`/transfer claude` follows the foreign-session resume flow introduced by Grok
...
4. **Inject** — build a *handoff prompt* (safety boundary + resolved session
   ...
   shows a slim `Transfer from Claude Code…` meta line instead of the raw
   handoff blob.
...
`/transfer codex` reads Codex CLI/VSCode rollout transcripts
...
- **Total transcript cap** — a session file larger than 32 MiB is rejected with
  a clear message (`too large for a transfer`) instead of being slurped whole.
...
`/transfer` executes all file I/O + parsing on a **background task**
(`spawn_blocking`), never on the TUI render thread. The slash input is not
echoed as a user card; the visible feedback is the slim transfer meta line plus
...
- `crates/coding-agent/src/agent/transfer/mod.rs` — Claude discovery, resolution,
  transcript reader, handoff-prompt builder (Rust port of the reference
  `session_reader.py`), plus shared helpers.
- `crates/coding-agent/src/agent/transfer/codex.rs` — Codex rollout reader,
  discovery, resolution, handoff-prompt builder.
- `crates/coding-agent/src/agent/transfer/tests.rs` (Claude) and
  `crates/coding-agent/src/agent/transfer/codex/tests.rs` (Codex) — unit tests
  (fixtures are written to a tempdir; never touch the real `~/.claude` /
  `~/.codex`).
- `crates/coding-agent/src/agent/slash_commands.rs` — `/transfer` registration
  + `SlashDispatch::Transfer`, arg completions (`claude`, `codex`).
...
- **[Implemented]** Claude Code → Elph transfer.
- **[Implemented]** Codex → Elph transfer (CLI/VSCode rollout transcripts).
