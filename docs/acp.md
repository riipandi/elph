...
- No audio prompts.
- File reads: `file://` prompt links are hydrated from disk. v1 also uses client `fs/read_text_file` when advertised. v2 has no client filesystem method surface; hydration is local to the agent. Writes stay local. Elph does **not** call client `fs/write_text_file`.
...
- After `session/new` / `resume` / `load` is **answered**, Elph sends `available_commands_update` immediately (builtins + already-loaded skills), then attaches client MCP and sends the catalog again. Clients such as Zed ignore command updates that arrive before they have the session id. `/tools` lists inactive (lazy) tools and loaded skills. `/reload` re-sends the slash catalog.
- ACP session open skips store GC and times out after 25s so a locked/slow datastore cannot hang `session/new` until the client closes stdio (`incoming_transport_closed`). Open work is isolated from the JSON-RPC I/O task so a panic answers with an error instead of killing the process.
...
- Tool progress is a **replace** `tool_call_update` of accumulated output, not `tool_call_content_chunk` of each harness delta. A `tool_call` / first `tool_call_update` is always sent before any later update so clients do not report `Tool call not found`.
...
- Prompt, slash, skill, and template failures are written into the session as agent text (plus `agent_message_chunk`) and then go idle. v1 still completes `session/prompt` with `stopReason` instead of a bare JSON-RPC error. A turn that finishes with no model text reports that explicitly. MCP discovery is capped at 12s so `/skill:…` cannot hang forever.
- Rate-limit / provider `Status` and `Retrying` events are forwarded as agent text (v1 chunks too). They are not dropped by the UI-stream drain. TUI-only “Press Ctrl+R” is rewritten for ACP.
...
| `session/new` | Spawned off I/O. Response first; then `available_commands_update`, MCP wait, catalog refresh, `config_option_update`. |
...
| `session/new` | Spawned off I/O. Response first; then `available_commands_update`, MCP wait, catalog refresh, full `config_option_update`. |
