...
// ---------------------------------------------------------------------------
// TypeScript object-literal helpers (only used by image model parsing)
// ---------------------------------------------------------------------------
...
fn ts_object_to_json(body: &str) -> Result<Value> {
    let mut body = body.trim().to_string();
    if !body.starts_with('{') {
        anyhow::bail!("expected object literal starting with '{{'");
    }
...
fn catalog_const_name(provider_id: &str) -> String {
