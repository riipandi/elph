...
Third-party libraries that bypass the `log` crate by writing straight to fd 2 — the Crawlberg/deno_core browser backend and the MCP (rmcp) client — are redirected to `{logs_dir}/crawlberg.log` on first use so their output stays out of the TUI. See [`logger::redirect_stderr_to_file`](../src/logger/mod.rs).
