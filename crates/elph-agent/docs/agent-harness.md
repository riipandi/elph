...
- `websearch`, `webfetch` — outbound HTTP (and optional [Crawlberg](https://crates.io/crates/crawlberg) browser fallback); no `ExecutionEnv` required. Enable the `tools-web` feature and register via `BuiltinToolsBuilder::all(env).build()` or `create_web_tools()`.
...
- Web tools (`websearch`, `webfetch`) with multi-engine ranking and Crawlberg fallback
