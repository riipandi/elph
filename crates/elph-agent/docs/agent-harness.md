...
A process-local `PlanModeTracker` (`Inactive` / `Pending` / `Active`) sits next to `CollaborationMode`. TUI Shift+Tab to Plan calls `arm_plan_mode()` (Pending, tools unchanged). The first user prompt or `enter_plan_mode()` activates Plan and applies the Plan tool filter (exploration plus workspace mutating tools). Multi-agent and mutating MCP tools stay out of the Plan set and are hard-blocked if invoked. Workspace mutating calls are not hard-blocked so the host can approve them one call at a time; a headless host should deny those calls (no approval UI). Implementation still requires plan confirmation. Resume seeds Active from a restored `CollaborationMode::Plan`.
